...
Figure \ref{fig:arduino_reading_latency_boxplot} shows the the latencies of both programs.
 These results are shown on a logarithmic scale. This is needed because both results contain outliers with very high values. These outliers are likely caused by transmission errors when receiving data from the Arduino. The \textit{Bulk} interface is used, providing data integrity but no timing guarantees. Consequently, data loss will be prevented, but will introduce high latencies as observed here.
...
\textbf{Largest Outlier (μs)} & 83052 & 75618 \\ \hline
